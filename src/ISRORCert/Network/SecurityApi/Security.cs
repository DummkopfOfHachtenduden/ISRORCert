    ...
    public class Security
    {
        ...
        private byte[] FormatPacket(ushort opcode, byte[] data, bool encrypted)
        {
            ...
            if (m_client_security == false && m_security_flags.security_bytes == 1)
            {
                ...
                byte sb1 = opcode == 0x1001 || _isTrusted ? (byte)0 : GenerateCountByte(true);
                writer.BaseStream.Seek(4, SeekOrigin.Begin);
                writer.Write(sb1);
                writer.Flush();

                byte sb2 = opcode == 0x1001 || _isTrusted ? (byte)0 : GenerateCheckByte(writer.GetBytes());
        ...
        private KeyValuePair<TransferBuffer, Packet> GetPacketToSend()
        {
            ...
            if (packet.Massive)
            {
                ...
                PacketWriter massive = new PacketWriter();
                PacketWriter massiveData = new PacketWriter();

                byte[] input_data = packet.GetBytes();

                ushort parts = (ushort)(input_data.Length / 4089 + (input_data.Length % 4089 != 0 ? 1 : 0));

                // Write the final header packet to the front of the packet
                PacketWriter massiveHeader = new PacketWriter();
                massiveHeader.Write((byte)1); // Header flag
                massiveHeader.Write((short)parts);
                massiveHeader.Write(packet.Opcode);
                massiveHeader.Write((byte)0); // Disable encryption
                massive.Write(FormatPacket(0x600D, massiveHeader.GetBytes(), false));
                ...
                while (workspace.Size > 0)
                {
                    ...
                    massiveData.Write(FormatPacket(0x600D, part_data.GetBytes(), false));
                }

                // Finish the large packet of all the data
                massive.Write(massiveData.GetBytes());

                // Return the collated data
                byte[] raw_bytes = massive.GetBytes();
                packet.Lock();
